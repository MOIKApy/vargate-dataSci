...
for ind, row in temp_df.iterrows():
    ol = [row['Date_mplt'], row['Open'], row['High'], row['Low'], row['Close'], row['Volume']]
    ohlc.append(ol)
...
fig, ax = plt.subplots(figsize=(12,8))
sns.tsplot(df.Close.values, time=df.Date_mplt.values, alpha=0.8, color=color[2], ax=ax)
ax.xaxis.set_major_locator(matdates.AutoDateLocator())
ax.xaxis.set_major_formatter(matdates.DateFormatter('%Y.%m.%d'))
fig.autofmt_xdate()
plt.xlabel('Date', fontsize=12)
...
for ind, file_name in enumerate(files_to_use):
    ...
    if ind == 0:
        df = pd.read_csv("./data/"+file_name, usecols=["Date", "Close"], parse_dates=["Date"])
        df.columns = ["Date", coin_name]
    else:
        temp_df = pd.read_csv("./data/"+file_name, usecols=["Date", "Close"], parse_dates=["Date"])
